...
//    function Equals(region: TGPRegion; g: TGPGraphics): BOOL; reintroduce;
    function Equals(region: TGPRegion; g: TGPGraphics): BOOL; reintroduce; // to avoid warning in Delphi 2010
...
//    function Equals(matrix: TGPMatrix): BOOL;
    function Equals(matrix: TGPMatrix): BOOL; reintroduce; // to avoid warning in Delphi 2010
    function GetLastStatus: TStatus;
  end;
...
\**************************************************************************)
  ...
  begin
    result := SetStatus(GdipGetMatrixElements(nativeMatrix, PSingle(@m)));
  end;
...
\**************************************************************************)
  ...
  begin
    SetStatus(GdipGetStringFormatDigitSubstitution(nativeFormat, PUINT(@result), nil));
  end;
...
//--------------------------------------------------------------------------
  ...
  begin
    SetStatus(GdipGetLineColors(GpLineGradient(nativeBrush), PARGB(@colors)));
    if (lastResult = Ok) then
    begin
      color1 := colors[0];
      color2 := colors[1];
...
\**************************************************************************)
  ...
  begin
    result := SetStatus(GdipGetFamilyName(nativeFamily, @str[0], language));
    name := str;
  end;
