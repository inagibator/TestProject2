...
uses
  ...
  K_FPathNameFr, K_Clib0;

type
  TK_FormCMUTLoadDBData3 = class(TN_BaseForm)
    GBAll: TGroupBox;
    ...
    GBFileStorage: TGroupBox;
    Img3DPathNameFrame: TK_FPathNameFrame;
    VideoPathNameFrame: TK_FPathNameFrame;
    ImgPathNameFrame: TK_FPathNameFrame;
    BtUndoCopy: TButton;
    ...
    procedure BtUndoCopyClick(Sender: TObject);
  private
    ...
    MergePatientIDChangeFName : string;
    CorrectPathExists : Boolean;

    CopyFilesListFName, CopyFilesListCurFName, 
    ...
    CopyFilesListCount, CopyFilesListNum,
    ...
    procedure CalcUndoCopyFilesContext;

  public
    { Public declarations }
    MergeCopyFSDataFName, MergeCopyFSDataCurStateFName : string;
    MergeCopyFSDataCount, MergeCopyFSDataStart, MergeCopyNFCount : Integer;
...
var
  K_FormCMUTLoadDBData3: TK_FormCMUTLoadDBData3;

procedure K_CMUTLoadDBData3Dlg();

implementation

uses K_CM0, K_CM1, K_FCMSupport, K_UDT2, K_FCMUTLoadDBData3FSClear,
     N_Lib0, N_Lib1, N_Types;
{$R *.dfm}

var MergeCopyFSDataPortion : Integer = 100;
//var MergeCopyFSDataPortion : Integer = 20;
//var CopyFilesListPortionCount : Integer = 100000;
//var CopyFilesListPortionCount : Integer = 50;
var CopyFilesListPortionCount : Integer = 50000;
...
begin
  ...
  K_FormCMUTLoadDBData3 := TK_FormCMUTLoadDBData3.Create(Application);
  ...
  begin
    ...
    N_Dump1Str( 'DAD>> Before K_FormCMUTLoadDBData3 show' );
    ShowModal();
    N_Dump1Str( 'DAD>> After K_FormCMUTLoadDBData3 show' );
    ...
    K_FormCMUTLoadDBData3 := nil;
  end;

end; // K_CMUTLoadDBData3Dlg

procedure TK_FormCMUTLoadDBData3.PathChange;
begin
  BtOpen.Enabled := PathNameFrame.mbPathName.Text <> '';
  BtLoad.Enabled := FALSE;
  BtCopy.Enabled := FALSE;
  BtUndoCopy.Enabled := FALSE;
  ...
  if not BtOpen.Enabled then Exit;
  
  if MergeRootPath = '' then
    MergeRootPath := PathNameFrame.mbPathName.Text;
  if (ImgPathNameFrame.mbPathName.Text = '') or
     SameText(MergeRootPath + K_ImgFolder, ImgPathNameFrame.mbPathName.Text) then
    ImgPathNameFrame.AddNameToTop(PathNameFrame.mbPathName.Text + K_ImgFolder );

  if (VideoPathNameFrame.mbPathName.Text = '') or
     SameText(MergeRootPath + K_VideoFolder, VideoPathNameFrame.mbPathName.Text) then
    VideoPathNameFrame.AddNameToTop(PathNameFrame.mbPathName.Text + K_VideoFolder );

  if (Img3DPathNameFrame.mbPathName.Text = '') or
     SameText(MergeRootPath + K_Img3DFolder, Img3DPathNameFrame.mbPathName.Text) then
    Img3DPathNameFrame.AddNameToTop(PathNameFrame.mbPathName.Text + K_Img3DFolder );

  MergeRootPath := PathNameFrame.mbPathName.Text;

end; // procedure TK_FormCMUTLoadDBData3.PathChange

procedure TK_FormCMUTLoadDBData3.DumpFSExisted( const ARelPath : string );
begin
  ...
  Img3DFList.Clear;
end; // procedure TK_FormCMUTLoadDBData3.DumpFSExisted

procedure TK_FormCMUTLoadDBData3.DumpFSNeeded( const ARelPath : string );
begin
 ProgressShowPrev := -1;
 with TK_CMEDDBAccess(K_CMEDAccess), CurDSet3 do
  ...
  AImg3DFList.Clear;
end; // procedure TK_FormCMUTLoadDBData3.DumpFSNeeded

procedure TK_FormCMUTLoadDBData3.LoadSlidesCopyFSDataTable();
begin
...
end; // procedure TK_FormCMUTLoadDBData3.LoadSlidesCopyFSDataTable

function TK_FormCMUTLoadDBData3.SaveCopyFSDataStateToFile( ) : Boolean;
begin
  Result := K_VFSaveIntDataToTextFile( MergeCopyFSDataCurStateFName, K_CMEDAccess.TmpStrings,
         [MergeCopyFSDataStart,MergeCopy2Count,
          MergeCopySCount,MergeCopyVCount,MergeCopy3Count,MergeCopyNFCount] );
end; // function TK_FormCMUTLoadDBData3.SaveCopyFSDataStateToFile

function TK_FormCMUTLoadDBData3.LoadCheckFSBeforeStateFromFile( ) : Boolean;
begin
  Result := K_VFLoadIntDataFromTextFile(MergeCheckFSBeforeFName, K_CMEDAccess.TmpStrings,
...
end; // function TK_FormCMUTLoadDBData3.LoadCopyFSDataStateFromFile

function TK_FormCMUTLoadDBData3.LoadCopyFSDataStateFromFile( ) : Boolean;
begin
  Result := K_VFLoadIntDataFromTextFile( MergeCopyFSDataCurStateFName, K_CMEDAccess.TmpStrings,
                                 [@MergeCopyFSDataStart,@MergeCopy2Count,
                                  @MergeCopySCount,@MergeCopyVCount,@MergeCopy3Count,@MergeCopyNFCount] );
  if not Result then
  begin
  ...
  N_Dump1Str( format( 'DAD>> Copy state is loaded: %d Image, %d Study, %d Video files and %d 3D folders for %d of %d Media Objects, %d Objects are WO files',
                                      [MergeCopy2Count, MergeCopySCount,
                                       MergeCopyVCount, MergeCopy3Count,
                                       MergeCopyFSDataStart - 1, MergeCopyFSDataCount, MergeCopyNFCount] ) );
end; // function TK_FormCMUTLoadDBData3.LoadCopyFSDataStateFromFile

function TK_FormCMUTLoadDBData3.GetTableCount( const ATableName : string  ) : Integer;
begin
...
end; // function TK_FormCMUTLoadDBData3.GetTableCount

procedure TK_FormCMUTLoadDBData3.CopyLogs();
var
  LogFName : string;
  RLogFName : string;
  LocFPath : string;
begin
  ...
  RLogFName := MergeInfoPath + LocFPath + ExtractFileName(LogFName);
  K_CopyFile( LogFName, RLogFName );
end; // procedure TK_FormCMUTLoadDBData3.CopyLogs

procedure TK_FormCMUTLoadDBData3.RebuildPathSettings;
begin
...
procedure TK_FormCMUTLoadDBData3.FormShow(Sender: TObject);
begin
...
procedure TK_FormCMUTLoadDBData3.BtOpenClick(Sender: TObject);
var
  UndoCopyPauseCount : Integer;
begin
  ...
  MergePatientIDChangeFName := MergeDBDataPath + 'PatientIDChange.dat';
  ...
  CopyFilesListFName := MergeInfoPath + 'T2\UndoFList';
  ...
  begin
    ...
    K_DeleteFile( CopyFilesListFName  + 'Counters.dat'  );
    K_DeleteFolderFiles( ExtractFilePath(CopyFilesListFName),
                         ExtractFileName(CopyFilesListFName) + '*.txt', [] );
    CopyFilesListCount := 0;
    CopyFilesListNum := 0;
    CopyFilesListCurFName := CopyFilesListFName + '.txt';
  end;
  ...
  RebuildPathSettings();
  CalcUndoCopyFilesContext();
  K_VFLoadIntDataFromTextFile( CopyFilesListFName  + 'Counters.dat',
                               K_CMEDAccess.TmpStrings, [@UndoCopyPauseCount] );
  if BtCopy.Enabled then
    BtCopy.Enabled := UndoCopyPauseCount = 0; // set BtCopy disabled if Undo was not finished
...
procedure TK_FormCMUTLoadDBData3.ChBLoadMediaClick(Sender: TObject);
begin
...
procedure TK_FormCMUTLoadDBData3.BtLoadClick(Sender: TObject);
var
//  ExtFilePat : string;
//  ErrRootPath : string;
  SavedCursor: TCursor;
...
begin
  ...
  begin
    K_AddStringToTextFile( MergeReportFName,
      format( #13#10'Merging Load DB data was started at %s',
    ...
    begin
      ...
      if not K_VFSaveIntDataToTextFile( MergeCheckFSBeforeFName, K_CMEDAccess.TmpStrings,
              [FSBeforeExisted,FSBeforeNeeded,
...
{}
    ...
    begin // Save SlidesCopyFSData
      ...
      CommandText := 'CREATE LOCAL TEMPORARY TABLE PAIDs3 (ID1 int, ID2 int) NOT TRANSACTIONAL;' + #10 +
                     ' LOAD TABLE PAIDs3' +
                     ' FROM ''' + MergePatientIDChangeFName + ''' FORMAT BCP;'#10 +
                     'CREATE LOCAL TEMPORARY TABLE SlidesCopyFSData ' +
                     '(OldID int, NewID int, OldPatID int, NewPatID int, DTCr datetime, SysInfo varchar(1000)) NOT TRANSACTIONAL;' + #10 +
            ...
            ' SELECT A.OldID, A.NewID, A.OldPatID, P.' + K_CMENDBSTFPatID +
                 ...
                 ' P ON A.NewID = P.' + K_CMENDBSTFSlideID + ';' + #10 +
            'UPDATE SlidesCopyFSData SET OldPatID = A.ID1 FROM PAIDs3 A WHERE OldPatID = A.ID2;';
...
//    BtLoad.Enabled := TRUE;
    ...
    K_AddStringToTextFile( MergeReportFName,
      format( 'Merging Load DB data was finished at %s',
              [ K_DateTimeToStr( Now(), 'yyyy-mm-dd_hh":"nn":"ss' )] ) );
...
procedure TK_FormCMUTLoadDBData3.BtCopyClick(Sender: TObject);
var
  ErrRootPath : string;
  ImgRootPath : string;
...
//  FilesPattern : string;
  ...
  VideoFileFlag, Img3DFileFlag : Boolean;
...
//  FStream : TFileStream;
  ...
  procedure SaveFileName( const AFName : string );
  begin
    if CopyFilesListCurFName = '' then
      CalcUndoCopyFilesContext();

    if FilesListObj = nil then
      FilesListObj := TK_DumpObj.Create(CopyFilesListCurFName)
    else
    if FilesListObj.DLogFname <> CopyFilesListCurFName then
      FilesListObj.DLogFname := CopyFilesListCurFName;

    if CopyFilesListPortionCount = CopyFilesListCount then
    begin
      CopyFilesListCount := 0;
      Inc( CopyFilesListNum );
      CopyFilesListCurFName := CopyFilesListFName + IntToStr( CopyFilesListNum ) + '.txt';
      FilesListObj.DLogFname := CopyFilesListCurFName;
    end;
    FilesListObj.DumpStr0( AFName );
    Inc(CopyFilesListCount);
  end; // procedure SaveFileName
    ...
    begin
      ...
      begin
        ...
        OldLocPath := K_CMSlideGetPatientFilesPathSegm( OldPatID ) +
                      OldLocPath;
        VideoFileFlag := Pos( 'IsMediaObj', SysInfo ) > 0;
        Img3DFileFlag := Pos( '3D', SysInfo ) > 0;
//        AddNamePat := '';
...
//          AddNamePat := '*';
        ...
        SrcBasePath := SrcBasePath + OldLocPath;
        if Img3DFileFlag then
        begin // 3D
          OldFName := SrcBasePath + K_CMSlideGetImg3DFolderName( SOldID );
          NewFName := DstBasePath + K_CMSlideGetImg3DFolderName( SNewID );
          SaveFileName( NewFName );
          if K_CopyFolderFiles( OldFName, NewFName ) then
          begin
            Inc(MergeCopy3Count);
            N_Dump1Str( format( 'Files %s >> %s copy',[OldFName,NewFName]) );
          end
          else
          begin
            Inc(MergeCopyNFCount);
            N_Dump1Str( format( 'DAD>> Files %s >> %s >> some copy error',[OldFName,NewFName]) );
          end;
        end   // 3D
        else
        begin // not 3D
//          CopyResCode := 0;
          if VideoFileFlag then
          begin // Video
            K_CMEDAGetSlideSysFieldsData( SysInfo, @WCMSDB );
            OldFName := SrcBasePath + K_CMSlideGetMediaFileNamePref( SOldID ) + WCMSDB.MediaFExt;
            NewFName := DstBasePath + K_CMSlideGetMediaFileNamePref( SNewID ) + WCMSDB.MediaFExt;
            SaveFileName( NewFName );
            CopyResCode := K_CopyFile( OldFName, NewFName );
            if CopyResCode = 0 then
              Inc(MergeCopyVCount);
          end   // if VideoFileFlag then
          else
          begin // if not VideoFileFlag then
            if SysInfo = '' then
            begin // Study
              OldFName := SrcBasePath + K_CMStudyGetFileName( SOldID );
              NewFName := DstBasePath + K_CMStudyGetFileName( SNewID );
              SaveFileName( NewFName );
              CopyResCode := K_CopyFile( OldFName, NewFName );
              if CopyResCode = 0 then
                Inc(MergeCopySCount);
            ...
            begin // Img
              // Try to copy file with DICOM attrs
              OldFName := SrcBasePath + K_CMSlideGetDCMAttrsFileName( SOldID );
              if FileExists(OldFName) then
              begin
                NewFName := DstBasePath + K_CMSlideGetDCMAttrsFileName( SNewID );
                SaveFileName( NewFName );
                CopyResCode := K_CopyFile( OldFName, NewFName );
                if CopyResCode = 0 then
                begin
                  N_Dump1Str( format( 'File %s >> %s copy',[OldFName,NewFName]) );
                  Inc( MergeCopy2Count);
                end
                else
                begin
                  Inc(MergeCopyNFCount);
                  N_Dump1Str( format( 'DAD>> File %s >> %s >> copy error = %d',[OldFName, NewFName,CopyResCode]) );
                end;
              end; // if FileExists(OldFName) then

              OldFName := SrcBasePath + 'RF_' + K_CMSlideGetFileIDPathSegm( SOldID );
              NewFName := DstBasePath + 'RF_' + K_CMSlideGetFileIDPathSegm( SNewID );
              if Pos( 'HasSrcImg', SysInfo ) > 0 then
              begin // SrcImage
                SaveFileName( NewFName + 'r.cmi' );
                CopyResCode := K_CopyFile( OldFName + 'r.cmi', NewFName + 'r.cmi' );
                if CopyResCode = 0 then
                begin
                  N_Dump1Str( format( 'File %s >> %s copy',[OldFName + 'r.cmi',NewFName + 'r.cmi']) );
                  Inc( MergeCopy2Count);
                end
                else
                begin
                  Inc(MergeCopyNFCount);
                  N_Dump1Str( format( 'DAD>> File %s >> %s >> copy error = %d',[OldFName + 'r.cmi', NewFName + 'r.cmi',CopyResCode]) );
                end;
              end; // if Pos( 'HasSrcImg', SysInfo ) > 0 then
              OldFName := OldFName + '.cmi';
              NewFName := NewFName + '.cmi';
              SaveFileName( NewFName );
              CopyResCode := K_CopyFile( OldFName, NewFName );
              if CopyResCode = 0 then
                Inc( MergeCopy2Count);
...
//sleep(500);  // debug
      ...
      N_Dump2Str( format( 'DAD>> Current Copy state: %d Image, %d Study, %d Video files and %d 3D folders for %d of %d Media Objects, %d Objects are WO files',
                                          [MergeCopy2Count, MergeCopySCount,
                                           MergeCopyVCount, MergeCopy3Count,
  ...
  K_FormCMSupport.ShowInfo( format( '%d Image, %d Study, %d Video files and %d 3D folders for %d of %d Media Objects are added, %d Objects are WO files',
                                    [MergeCopy2Count, MergeCopySCount,
                                     MergeCopyVCount, MergeCopy3Count,
                                     MergeCopyFSDataStart-1,MergeCopyFSDataCount,
                                     MergeCopyNFCount] ), 'TK_FormCMUTLoadDBData.BtCopyClick >>' );
  CopyLogs();
  ...
  begin // Pause is set not from CloseQuery
    ...
    BtCopy.Enabled := TRUE;
    BtUndoCopy.Enabled := CopyFilesListCount > 0;
    Exit;
  end;
  ...
  begin
    K_AddStringToTextFile( MergeReportFName,
      format( 'Total to Target:'#13#10 +
              ...
              '%6d DB media objects are without files or 3D folders.',
               [MergeCopyFSDataCount, MergeCopy2Count + MergeCopySCount + MergeCopyVCount + MergeCopy3Count, MergeCopyNFCount] ) );
    K_AddStringToTextFile( MergeReportFName,
      format( '(Files copy details: Images=%d, Studies=%d, Video=%d, 3D=%d)',
               [MergeCopy2Count, MergeCopySCount, MergeCopyVCount, MergeCopy3Count] ) );
  end;
  ...
  /////////////////////////////////////////////
    ...
    if (FSAfterExtra = FSBeforeExtra) and
       (FSAfterMissing = FSBeforeMissing + MergeCopyNFCount) and
       (FSAfterExisted - FSBeforeExisted =
    ...
    begin
      ...
      if FSAfterMissing > FSBeforeMissing + MergeCopyNFCount then
  ...
  Self.Close;
end; // procedure TK_FormCMUTLoadDBData3.BtCopyClick

procedure TK_FormCMUTLoadDBData3.BtUndoCopyClick(Sender: TObject);
begin
//
  with TK_FormCMUTLoadDBData3FSClear.Create( Application ) do
  begin
    FilesListPortionCount := CopyFilesListPortionCount;
    FilesListFName := CopyFilesListFName;
    BaseFormInit( nil, '', [rspfMFRect,rspfCenter], [rspfAppWAR,rspfShiftAll] );
    ShowModal();
    if AllCount = ProcCount then
    begin // Clear Undo Context
      K_DeleteFolderFiles( ExtractFilePath(CopyFilesListFName),
                           ExtractFileName(CopyFilesListFName) + '*.txt', [] );
      CopyFilesListCount := 0;
      CopyFilesListNum   := 0;
  ...
  CopyLogs();
end; // procedure TK_FormCMUTLoadDBData3.BtUndoCopyClick

procedure TK_FormCMUTLoadDBData3.CurStateToMemIni;
begin
  inherited;
  N_ComboBoxToMemIni( 'CMSLoadDBDataPathsHistory', PathNameFrame.mbPathName );
  N_ComboBoxToMemIni( 'CMSLoadDBDataImgPathsHistory', ImgPathNameFrame.mbPathName );
  N_ComboBoxToMemIni( 'CMSLoadDBDataVideoPathsHistory', VideoPathNameFrame.mbPathName );
  N_ComboBoxToMemIni( 'CMSLoadDBDataImg3DPathsHistory', Img3DPathNameFrame.mbPathName );
end; // procedure TK_FormCMUTLoadDBData.CurStateToMemIni

procedure TK_FormCMUTLoadDBData3.MemIniToCurState;
begin
  inherited;
  ...
  N_MemIniToComboBox( 'CMSLoadDBDataImgPathsHistory', ImgPathNameFrame.mbPathName );
  ImgPathNameFrame.AddNameToTop( ImgPathNameFrame.mbPathName.Text );

  N_MemIniToComboBox( 'CMSLoadDBDataVideoPathsHistory', VideoPathNameFrame.mbPathName );
  VideoPathNameFrame.AddNameToTop( VideoPathNameFrame.mbPathName.Text );

  N_MemIniToComboBox( 'CMSLoadDBDataImg3DPathsHistory', Img3DPathNameFrame.mbPathName );
  Img3DPathNameFrame.AddNameToTop( Img3DPathNameFrame.mbPathName.Text );

end; // procedure TK_FormCMUTLoadDBData.MemIniToCurState

procedure TK_FormCMUTLoadDBData3.ShowProgress( AllCount, CurCount: Integer);
begin
...
procedure TK_FormCMUTLoadDBData3.FormCloseQuery(Sender: TObject;
...
end; // procedure TK_FormCMUTLoadDBData3.FormCloseQuery

procedure TK_FormCMUTLoadDBData3.CalcUndoCopyFilesContext;
begin
  /////////////////////////
  // Calc Files List Count
  //
  CopyFilesListNum := K_CountFolderFiles( ExtractFilePath(CopyFilesListFName),
                                          'UndoFList*.txt', [] );
  CopyFilesListCurFName := CopyFilesListFName;
  if CopyFilesListNum > 0 then
  begin
    Dec(CopyFilesListNum);
    if CopyFilesListNum > 0 then
      CopyFilesListCurFName := CopyFilesListCurFName + IntToStr( CopyFilesListNum );
    CopyFilesListCurFName := CopyFilesListCurFName + '.txt';
    with K_CMEDAccess.TmpStrings do
    begin
      Clear;
      LoadFromFile(CopyFilesListCurFName);
      CopyFilesListCount := Count;
      Clear;
    end;
  end // if CopyFilesListNum > 0 then
  else
  begin
    CopyFilesListCount := 0;
    CopyFilesListCurFName := CopyFilesListCurFName + '.txt';
  ...
  BtUndoCopy.Enabled := (CopyFilesListNum > 0) or (CopyFilesListCount > 0);
  //
  // Calc Files List Count
  /////////////////////////
end; // procedure TK_FormCMUTLoadDBData3.CalcUndoCopyFilesContext;
