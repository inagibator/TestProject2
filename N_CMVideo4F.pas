unit N_CMVideo4F;
// Form for Video Capturing in CMS Project (Mode #2)
...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, ComCtrls, ToolWin, Types,
  K_Types, K_CM0, N_Video,
  N_Types, N_Lib0, N_Lib1, N_Gra1, N_Gra2, N_BaseF, N_Video4, N_Rast1Fr,
  N_DGrid, N_CM2, N_CM1, DirectShow9, Grids, Buttons, Menus;

type
  TN_CMVFPrepFlags = Set Of ( cmvfpfInitialCall, cmvfpfInitSizes,
                                                                 cmvfpfRecord );

type TN_CMVideo4Form = class( TN_BaseForm )
  MainPanel:            TPanel;
  RightPanel:           TPanel;
  ButtonsPanel:         TPanel;
  Label2:               TLabel;
  ...
  cbFlipHor:            TCheckBox;
  CmBCaptureButton:     TComboBox;
  CmBCameraButtonThreshold: TComboBox;
  lbCaptureButton:      TLabel;
  lbCameraButtonThreshold: TLabel;
  TimerError:           TTimer;
  TimerStillPin:        TTimer;
  cbFormat:             TComboBox;

  // ****************  TN_CMVideo4Form class handlers  ******************
  ...
  procedure CmBCameraButtonThresholdChange ( Sender: TObject );
  procedure CmBCaptureButtonChange         ( Sender: TObject );
  procedure TimerErrorTimer  ( Sender: TObject );
  procedure TimerStillPinTimer             ( Sender: TObject );
...
public
  ...
  CMVFVideoCapt4:    TN_VideoCaptObj4; // main Video Capturing Object
  ...
  CMVFDumpCounter: Integer; // counting same logs

  XWidth, YHeight: Integer; // width and height of the image capturing

  CMVFTakenFlag, CMVFStopped:  Boolean;
  CMVFTimerCount: Integer; // to wait before capture
  ...
  procedure CMVFSetNewSize         ( ASizeStr: string; AFormatGUIDStr: string );
...
end; // type TN_CMVideo4Form = class( TN_BaseForm )

  // *********** Global Procedures  *****************************

function N_CreateCMVideo4Form( AOwner: TN_BaseForm; // ADevNamesList: TStrings;
                               APProfile: TK_PCMVideoProfile ): TN_CMVideo4Form;

var
  N_CMVideo4Form: TN_CMVideo4Form; // used in N_CMSCreateDumpFiles
  N_TempThreshold: Integer;        // for qi
  N_PreviousMode: Integer;         // still or video item from previous mode
...
// OnShow Self handler
//
procedure TN_CMVideo4Form.FormShow( Sender: TObject );
var
  i: Integer;
  StringTemp: string;
  SlideInd: Integer;
begin
  CMVFVideoCapt4.VCOError := False;

  N_Video4.N_StillPinPressed := False;

  CMVFTakenFlag := True; // not in other modes
  ...
  begin
    ...
    N_Dump2Str(
              'TN_CMVideo4Form.FormShow before CMDPStrPar2[4], CMDPStrPar2 = ' +
    ...
    N_Dump2Str(
              'TN_CMVideo4Form.FormShow after CMDPStrPar2[4], CMDPStrPar2 = ' +
                                                     CMVFPProfile.CMDPStrPar2 );

    if CMVFPProfile.CMDPStrPar2[1] = '1' then
      cbFlipHor.Checked := True
    else
      cbFlipHor.Checked := False;
  ...
  if CMVFCameraBtnDevInd = N_FPCB_QI then
  begin
    CmBCaptureButton.ItemIndex := StrToInt( CMVFPProfile.CMDPStrPar2[2] );
    CmBCameraButtonThreshold.ItemIndex :=
                                        StrToInt( CMVFPProfile.CMDPStrPar2[3] );
    lbCaptureButton.Visible := True;
    CmBCaptureButton.Visible := True;
    lbCameraButtonThreshold.Visible := True;
    CmBCameraButtonThreshold.Visible := True;

    if CmBCaptureButton.ItemIndex = 0 then
    begin
      CmBCameraButtonThreshold.Enabled := False;
      lbCameraButtonThreshold.Enabled := False;
    end else
    begin
      CmBCameraButtonThreshold.Enabled := True;
      lbCameraButtonThreshold.Enabled := True;
    end;
  end;
  ...
  CMVFVideoCapt4.BUFFER_WINDOW         := 3000;
  CMVFVideoCapt4.MAX_KEY_FRAME_SPACING := 80000000;
  CMVFVideoCapt4.BitrateNum            := 4;
  CMVFVideoCapt4.AvgTimePerFrame       := 333333;
  CMVFVideoCapt4.SaveFileName := K_ExpandFileName( '(#WrkFiles#)LastFilm.wmv' );

end; // procedure TN_CMVideo4Form.FormShow

// *********************************************** TN_CMVideo4Form.FormClose ***
...
//
procedure TN_CMVideo4Form.FormClose( Sender: TObject;
...
begin
  ...
  begin
    ModalResult := idRetry;
    N_Slides.SIASlides := N_CMVideo4Form.CMVFSSASlides;
  ...
  begin
    ...
    CMVFVideoCapt4.VCOStopRecording();
  end;

  for i := 0 to Length(N_CMVideo4Form.CMVFSSASlides)-1 do
  begin
    N_CMVideo4Form.CMVFSSASlides[i].P^.CMSDB.DCMModality := 'ES';
  end;

  // Stop the graph first
  CMVFVideoCapt4.VCOIMediaControl := Nil;
  CMVFVideoCapt4.VCoCoRes := CMVFVideoCapt4.VCOIGraphBuilder.QueryInterface
                         ( IID_IMediaControl, CMVFVideoCapt4.VCOIMediaControl );
  if Succeeded( CMVFVideoCapt4.VCoCoRes ) then
  begin
    CMVFVideoCapt4.VCoCoRes := CMVFVideoCapt4.VCOIMediaControl.Stop;
    CMVFVideoCapt4.VCOIMediaControl := Nil;
  end;

  if Failed( CMVFVideoCapt4.VCoCoRes ) then
  begin
    ShowMessage( Format( 'Error %x: Cannot stop preview graph',
                                                  [ CMVFVideoCapt4.VCoCoRes ]));
    Exit;
  end;

  // Delete graph and filters (added for new modes (2, 3) only)
  if ( CMVFPProfile.CMDPStrPar1 = '3' ) then
  begin
    CMVFVideoCapt4.VCOClearGraph();
    CMVFVideoCapt4.VCOFreeCapFilters();
  end;
  ...
  begin
    ...
    CMVFormatGUIDString := CMVFVideoCapt4.VCOFormatList[cbFormat.ItemIndex];
    CMVCMode       := TK_CMVCMode( N_PreviousMode ); // so, it will return to the previous one
    CMPreviewMode  := CMVFNewPreviewMode; // set by last tbnPMi Toolbutton Click
    CMNumClicksInd := rgOneTwoClicks.ItemIndex;

    N_Dump2Str(
             'TN_CMVideo4Form.FormClose before CMDPStrPar2[1], CMDPStrPar2 = ' +
                                                      CMVFPProfile.CMDPStrPar2);
    // Save the Flip combobox state into CMDPStrPar2
    if CMVFVideoCapt4.VCOFlip = True then
      CMDPStrPar2[1] := '1'
    else
      CMDPStrPar2[1] := '0';

    N_Dump2Str(
             'TN_CMVideo4Form.FormClose before CMDPStrPar2[2], CMDPStrPar2 = ' +
                                                      CMVFPProfile.CMDPStrPar2);

    // save qi parameters
    TempStr := IntToStr( CmBCaptureButton.ItemIndex );
    CMDPStrPar2[2] := TempStr[1];

    N_Dump2Str(
             'TN_CMVideo4Form.FormClose before CMDPStrPar2[3], CMDPStrPar2 = ' +
                                                      CMVFPProfile.CMDPStrPar2);

    TempStr := IntToStr( CmBCameraButtonThreshold.ItemIndex );
    CMDPStrPar2[3] := TempStr[1];
    N_Dump2Str(
             'TN_CMVideo4Form.FormClose after CMDPStrPar2[2], CMDPStrPar2 = ' +
                                                      CMVFPProfile.CMDPStrPar2);
  ...
  N_CMVideo4Form := Nil;
end; // procedure TN_CMVideo4Form.FormClose

//**************************************** TN_CMVideo4Form.cbCaptSizeChange ***
...
//
procedure TN_CMVideo4Form.cbCaptSizeChange( Sender: TObject );
begin
  N_Dump2Str( ' cbCaptSize : ' + cbCaptSize.Text );
  CMVFPrepare([]);
end; // procedure TN_CMVideo4Form.cbCaptSizeChange

//****************************************** TN_CMVideo4Form.cbFlipHorClick ***
//
...
// Sender - Event Sender (Action, MenuItem or ToolButton)
//
procedure TN_CMVideo4Form.cbFlipHorClick( Sender: TObject );
begin
  inherited;

  CMVFVideoCapt4.VCOFlip := cbFlipHor.Checked;
  CMVFPrepare([]);
end; // procedure TN_CMVideo4Form.cbFlipHorClick( Sender: TObject );
...
// Sender - Event Sender (Action, MenuItem or ToolButton)
//
procedure TN_CMVideo4Form.cbFormatChange( Sender: TObject );
begin
  N_Dump2Str( ' Format : ' + cbFormat.Text );
  CMVFPrepare([]);
end; // procedure TN_CMVideo4Form.cbFormatChange(Sender: TObject);


//**************************************** TN_CMVideo4Form.cbCaptModeChange ***
...
//
procedure TN_CMVideo4Form.cbCaptModeChange( Sender: TObject );
begin
...
end; // procedure TN_CMVideo4Form.cbCaptModeChange

//*********************************** TN_CMVideo4Form.bnStreamSettingsClick ***
...
//
procedure TN_CMVideo4Form.bnStreamSettingsClick( Sender: TObject );
var
  VFInfo: TN_VideoFileInfo;
begin
  with CMVFVideoCapt4 do
  begin
    if N_KeyIsDown( VK_SHIFT ) then // debug - just collect info about VCOVCaptDevName capabilities
    begin

      if ( CMVFPProfile.CMDPStrPar1 = '3' ) then // for Mode 3
  ...
  end; // with CMVFVideoCapt4 do
end; // procedure TN_CMVideo4Form.bnStreamSettingsClick

//******************************* TN_CMVideo4Form.bnCaptFilterSettingsClick ***
...
//
procedure TN_CMVideo4Form.bnCaptFilterSettingsClick( Sender: TObject );
var
  ...
  begin
    ...
    begin
      ...
      K_CMShowMessageDlg( IntToStr( VCOIError ) +
                  ' - 1, Unexpected DirectShow Error5 - ' + VCOSError, mtWarning );
    end;

    VCOShowFilterDialog( VCOIVideoCaptFilter, VCOWindowHandle );
    if not VCOVFWStatus then
    begin
      VCOEnumVideoSizes( cbCaptSize.Items, cbFormat.Items );
      CMVFPrepare([]);
      CMVFSetNewSize('', '');
    end;
  end; // with CMVFVideoCapt4 do
end; // procedure TN_CMVideo4Form.bnCaptFilterSettingsClick

//********************************* TN_CMVideo4Form.bnCrossBarSettingsClick ***
...
//
procedure TN_CMVideo4Form.bnCrossBarSettingsClick( Sender: TObject );
var
...
end; // procedure TN_CMVideo4Form.bnCrossBarSettingsClick

//***************************************** TN_CMVideo4Form.bnStopLiveClick ***
...
//
procedure TN_CMVideo4Form.bnStopLiveClick( Sender: TObject );
var
  SError: string;
label ShowError;
begin
  ...
  begin
    ...
    ShowError: // **************************
    SError := IntToStr( VCOIError ) + ' - 2, Unexpected DirectShow Error5 - ' +
                                                                      VCOSError;
    N_Dump1Str( SError );
    K_CMShowMessageDlg( SError, mtWarning );
  end; // with CMVFVideoCapt4 do
end; // procedure TN_CMVideo4Form.bnStopLiveClick

//******************************************* TN_CMVideo4Form.bnRecordClick ***
...
//
procedure TN_CMVideo4Form.bnRecordClick( Sender: TObject );
begin
...
end; // procedure TN_CMVideo4Form.bnRecordClick

//********************************************* TN_CMVideo4Form.bnStopClick ***
...
//
procedure TN_CMVideo4Form.bnStopClick( Sender: TObject );
var
  SlideInd:    Integer;
  SError:      string;
...
begin
  ...
  begin
    ...
    FSlide := K_CMSlideCreateForMediaObject('.wmv');// '.avi' );

    VCOIMediaControl.Stop;
    WMediaFName := FSlide.GetMediaFileClientName(); // TMP MediaFileName

    WCaptMediaFName := SaveFileName;//VCOFileName;            // wmv name added
    ...
    begin }
      ...
      N_b:= CopyFile( @N_StringToWide(WCaptMediaFName)[1], @N_StringToWide(WMediaFName)[1], True  );
      N_Dump1Str( 'Copy result = ' + BoolToStr(N_b ));
    ...
    N_Dump1Str( 'After CopyCaptureFile = ' + IntToStr(VCOCoRes) );
    ...
    ShowError: // **************************
    SError := IntToStr( VCOIError ) + ' - 3, Unexpected DirectShow Error5 - ' +
                                                                      VCOSError;
    N_Dump1Str( SError );
    K_CMShowMessageDlg( SError, mtWarning );
  ...
  CMVFControlControls( True ); // Enable ComboBoxes and DriverSettings Buttons
  Screen.Cursor := SavedCursor;
end; // procedure TN_CMVideo4Form.bnStopClick

//********************************************* TN_CMVideo4Form.tbnPMNClick ***
...
//
procedure TN_CMVideo4Form.tbnPMNClick( Sender: TObject );
var
...
end; // procedure TN_CMVideo4Form.tbnPMNClick

//************************************** TN_CMVideo4Form.TimerStillPinTimer ***
// Waiting for still pin button callback
//
//     Parameters:
// ASender - ToolButton just clicked
//
procedure TN_CMVideo4Form.TimerStillPinTimer( Sender: TObject );
begin
  inherited;
  TimerStillPin.Enabled := False;

  if N_Video4.N_StillPinPressed and not CMVFStopped then // from callback
  begin
    N_Dump1Str( 'TimerStillPin start button' );
    CMVFFPCBDown( 1 ); // Capture button is treated as Key 1
    CMVFFPCBUp( 1 );   // Capture button is treated as Key 1

    N_Video4.N_StillPinPressed := False;
    N_Dump1Str( 'TimerStillPin end button' );
  end;

  if N_Video4.N_StillPinPressed and CMVFStopped then // from callback and when it's started after stop
  begin
    N_Dump1Str( 'Callback after Stop button' );
    N_Video4.N_StillPinPressed := False;
    CMVFStopped := False;
  end;

  TimerStillPin.Enabled := True;
end; // procedure TN_CMVideo4Form.TimerStillPinTimer

//***************************************** TN_CMVideo4Form.TimerErrorTimer ***
...
//
procedure TN_CMVideo4Form.TimerErrorTimer( Sender: TObject );
var
  Devices: TStringList;
  IError: Integer;
begin
  //N_Dump1Str( 'Start TN_CMVideo4Form.TimerErrorTimer' );
  TimerError.Enabled := False;
  inherited;

  Inc( CMVFTimerCount );
  if CMVFTimerCount > 10 then
    CMVFTimerCount := 10; // just in case
  ...
  if not CMVFVideoCapt4.VCOError then // if the first time, so it won't work twice
    ...
    CMVFVideoCapt4.VCOError := True; // worked already
    N_Dump1Str( 'Error = True' );

    //VideoPanel.Repaint; // return it back to the code if needed to clear the freeze image after disconnect
  end;

  // ***** reenable video
  if CMVFVideoCapt4.VCOError then
  ...
  TimerError.Enabled := True;
  //N_Dump1Str( 'End TN_CMVideo4Form.TimerErrorTimer' );
end; // procedure TN_CMVideo4Form.TimerErrorTimer

//***************************************** TN_CMVideo4Form.LeftPanelResize ***
...
//
procedure TN_CMVideo4Form.LeftPanelResize( Sender: TObject );
var
...
begin
  ...
  begin
    ...
    if CMVFTakenFlag then // not in other modes
      CMVFSetVisibleVideoRect();
  end; // else //************* update Video Panel size and position
end; // procedure TN_CMVideo4Form.LeftPanelResize

//************************************* TN_CMVideo4Form.rgOneTwoClicksClick ***
...
//
procedure TN_CMVideo4Form.rgOneTwoClicksClick( Sender: TObject );
begin
  ...
  if CMVFIsFreezed then
    CMVFSkipAndCont();
end; // procedure TN_CMVideo4Form.rgOneTwoClicksClick

//************************************** TN_CMVideo4Form.bnCaptureMouseDown ***
// Freeze if needed
//
//     Parameters:
// Sender - Event Sender (Action, MenuItem or ToolButton)
...
//
procedure TN_CMVideo4Form.bnCaptureMouseDown( Sender: TObject;
                      Button: TMouseButton; Shift: TShiftState; X, Y: Integer );
begin
  N_Dump2Str( 'bnCaptureMouseDown' );
  if CMVFVideoCapt4.VCOSampleGrabberFlag then
    N_StillPinPressed := True
  else
    CMVFFPCBDown( 1 ); // Capture button is treated as Key 1
end; // procedure TN_CMVideo4Form.bnCaptureMouseDown

//**************************************** TN_CMVideo4Form.bnCaptureMouseUp ***
// Save if needed
//
//     Parameters:
// Sender - Event Sender (Action, MenuItem or ToolButton)
...
//
procedure TN_CMVideo4Form.bnCaptureMouseUp( Sender: TObject;
                      Button: TMouseButton; Shift: TShiftState; X, Y: Integer );
begin
  N_Dump2Str( 'bnCaptureMouseUp' );
  if CMVFVideoCapt4.VCOSampleGrabberFlag then
  begin
    N_Dump1Str('Capture MouseUp with StillPin');
  end
  else
    CMVFFPCBUp( 1 ); // Capture button is treated as Key 1
end; // procedure TN_CMVideo4Form.bnCaptureMouseUp

//********************************************* TN_CMVideo4Form.FormKeyDown ***
...
//
procedure TN_CMVideo4Form.FormKeyDown( Sender: TObject; var Key: Word;
                                                           Shift: TShiftState );
var
...
begin
  ...
  begin
    ...
    CMVFVideoCapt4.VCOEnumFilters2( SL );
...
end; // procedure TN_CMVideo4Form.FormKeyDown

//*********************************************** TN_CMVideo4Form.FormKeyUp ***
...
// Self (Form) OnKeyDown Handler
//
procedure TN_CMVideo4Form.FormKeyUp( Sender: TObject; var Key: Word;
                                                           Shift: TShiftState );
begin
...
end; // procedure TN_CMVideo4Form.FormKeyUp

//**************************************** TN_CMVideo4Form.RecordTimerTimer ***
...
//
procedure TN_CMVideo4Form.RecordTimerTimer( Sender: TObject );
begin
...
end; // procedure TN_CMVideo4Form.RecordTimerTimer

//**************************************** TN_CMVideo4Form.ExtDLLTimerTimer ***
...
//
procedure TN_CMVideo4Form.ExtDLLTimerTimer( Sender: TObject );
var
...
begin
  ...
  begin
    ...
    if CMVFVideoCapt4.VCOIMediaControl <> Nil then // so it won't crash
    if CMVFVideoCapt4.VCOGetGraphState() = 2 then // graph is running
    begin
      if CMVFDumpCounter = 0 then // only the first time
        N_Dump1Str('Use Camera Button Device started');
      Inc( CMVFDumpCounter );
      ...
      if ( ResCode <> 0 ) or ( CurState <> PrevState ) then
      begin
        N_Dump1Str('Timer worked ' + IntToStr(CMVFDumpCounter) + ' times');
        CMVFDumpCounter := 0; // start counting again
      ...
      begin
        ...
        N_Dump1Str(' if ResCode <> 0 then // Camera Button Device error');
      end
      ...
      if CMVFDumpCounter = 0 then // only the first time
        N_Dump1Str('Use Camera Button Device ended');
      ...
      begin
        ...
        begin
          if FPCBDUCloseMode = 0 then
            ModalResult := mrOK;
        end else if ResCode = N_DU_CAMERA_TAKEN then // =3
        begin
          CMVFTakenFlag := True; //***** not in the other modes
          ...
          cbCaptSize.Enabled         := True;
          cbFormat.Enabled           := True;

          WrkString := CMVFPProfile^.CMDPStrPar2[6];

          if (WrkString = '0') or (WrkString = '1') then
          begin
            CMVFVideoCapt4.VCOSetPreviewWindowSize2;
          end;
          if WrkString = '2' then
          begin
            CMVFVideoCapt4.VCOSetPreviewWindowSize3;
          end;
        end

        else if ResCode = N_DU_SAVE_AND_UNFREEZE then // =1
...
end; // procedure TN_CMVideo4Form.ExtDLLTimerTimer


//****************  TN_CMVideo4Form class public methods  *************

//************************************* TN_CMVideo4Form.CMVFShowNumCaptured ***
// Update NumCaptLabel Caption - Show Number of Captured Still Images and Videos
//
procedure TN_CMVideo4Form.CMVFShowNumCaptured();
begin
  NumCaptLabel.Caption := Format( 'Stills: %d    Video: %d', [ CMVFNumStills,
    CMVFNumVideos ]);
end; // end of procedure TN_CMVideo4Form.CMVFShowNumCaptured

//********************************************* TN_CMVideo4Form.CMVFPrepare ***
...
//
procedure TN_CMVideo4Form.CMVFPrepare( APrepFlags: TN_CMVFPrepFlags );
var
  Str, NeededSizeStr, NeededFormatGUIDString, StringTemp: string;
  DriverStreamSize: TPoint;
  ItemIndex, i: integer;
  CurRect, NeededRect: TRect;
label ClearCMVFVideoCapt4, StopSearch;
begin
  ...
  begin
    ...
    NeededFormatGUIDString := CMVFPProfile^.CMVFormatGUIDString;    
    N_Dump1Str('Needed 1 = '+NeededFormatGUIDString);
  end
  else // call from cbCaptxxx.OnChange
  begin
    ...
    if cbFormat.ItemIndex <> -1 then
      NeededFormatGUIDString := CMVFVideoCapt4.VCOFormatList[cbFormat.ItemIndex]
    else
      NeededFormatGUIDString := CMVFVideoCapt4.VCOFormatList[0];

      N_Dump1Str('Needed = '+NeededFormatGUIDString);
  ...
  begin
    ...
    N_PreviousMode := cbCaptMode.ItemIndex;
    cbCaptMode.ItemIndex := 1;
    ...
    begin
      ...
      N_Dump1Str( 'ExtDLLTimer.Enabled := True;' );
    ...
    begin
      ...
      bnRecord.Enabled           := True;
      bnStop.Enabled             := False;
      StillControlsPanel.Visible := True;//False;
    ...
    if ( CMVFPProfile.CMDPStrPar1 = '3' ) then // for Mode 2
    begin
    ...
    VCOSupportCrossbarInfo := CMVFVideoCapt4.VCOGetCrossbarFilterInfo();
    N_Dump1Str( 'After looking for crossbar' );
    // Activate / Deactivate Crossbar
    if VCOSupportCrossbarInfo = False then
    ...
    CMVFSetNewSize( NeededSizeStr, NeededFormatGUIDString ); // try to set needed Size
    ...
    N_Dump1Str( 'Before preparing a graph' );
    if cbCaptMode.ItemIndex = 0 then // initialize "Still Images" mode
    begin
      if ( CMVFPProfile.CMDPStrPar1 = '3' ) then // for Mode 3
        VCOPrepGrabGraph2();
        N_Dump1Str( 'Still graph prepared' );
    ...
    begin
      VCOPrepVideoFiles();
      if ( CMVFPProfile.CMDPStrPar1 = '3' ) then // for Mode 3
        if cmvfpfRecord in APrepFlags then
        begin
          {$IFNDEF VER150} //***** not Delphi 7
          VCOPrepRecordGraph2();
          {$ENDIF VER150} //****** no code in Delphi 7
    ...
    N_Dump1Str( 'Before initializing' );
    ...
    // get parameters of a preview to save an image
    ...
    ClearCMVFVideoCapt4: // ***** something wrong, clear all Objects
    ...
    Str := CMVFPProfile^.CMDPProductName +
                ' is not plugged in or not working properly! (Error=' + IntToStr
                                             ( CMVFVideoCapt4.VCOIError ) + ')';
    VideoPanel.Caption := Str;
  end; // with CMVFVideoCapt4 do
end; // end of procedure TN_CMVideo4Form.CMVFPrepare

//************************************* TN_CMVideo4Form.CMVFControlControls ***
...
//
procedure TN_CMVideo4Form.CMVFControlControls( AEnable: boolean );
begin
  ...
  cbFormat.Enabled         := AEnable;
end; // end of procedure TN_CMVideo4Form.CMVFControlControls

//****************************************** TN_CMVideo4Form.CMVFSetNewSize ***
...
//
procedure TN_CMVideo4Form.CMVFSetNewSize( ASizeStr: string; AFormatGUIDStr: string );
var
  i, Ind, FormatInd: Integer;
  NewSizeStr:   string;
  NewVideoSize: TPoint;
begin
  with CMVFVideoCapt4 do
  begin
    ...
    if AFormatGUIDStr = '' then
      AFormatGUIDStr := VCOFormatList[0];

    with cbFormat do
    begin
      FormatInd := VCOFormatList.IndexOf(AFormatGUIDStr); // search for GUIDs
      if FormatInd <> -1 then
        ItemIndex := FormatInd
      else
        ItemIndex := 0;
    ...
    VCOSetVideoSize( ASizeStr, AFormatGUIDStr, NewVideoSize );
    if VCOIError <> 0 then
      Exit;
...
end; // procedure TN_CMVideo4Form.CMVFSetNewSize

//********************************* TN_CMVideo4Form.CMVFSetVisibleVideoRect ***
...
//
procedure TN_CMVideo4Form.CMVFSetVisibleVideoRect();
var
  WrkString: string;
begin
  if CMVFVideoCapt4 = Nil then
  ...
  begin
    ...
    begin
      if ( CMVFPProfile.CMDPStrPar1 = '3' ) then
      begin
        //WrkString := N_MemIniToString( 'CMS_UserDeb', 'Renderer', '2' ); // reading from string of resolutions from ini

        WrkString := CMVFPProfile^.CMDPStrPar2[6];

        if (WrkString = '0') or (WrkString = '1') then
        begin
          VCOSetPreviewWindowSize2;
        end;
        if WrkString = '2' then
        begin
          VCOSetPreviewWindowSize3;
        end;

      end;
    end;
  end; // with CMVFVideoCapt4 do
end; // end of TN_CMVideo4Form.CMVFSetVisibleVideoRect

//******************************************* TN_CMVideo4Form.CMVFDrawThumb ***
...
//
procedure TN_CMVideo4Form.CMVFDrawThumb( ADGObj: TN_DGridBase; AInd: Integer;
                                                           const ARect: TRect );
begin
...
end; // end of TN_CMVideo4Form.CMVFDrawThumb

//**************************************** TN_CMVideo4Form.CMVFGetThumbSize ***
...
//
procedure TN_CMVideo4Form.CMVFGetThumbSize( ADGObj: TN_DGridBase; AInd: Integer;
        AInpSize: TPoint; out AOutSize, AMinSize, APrefSize, AMaxSize: TPoint );
var
  Slide:     TN_UDCMSlide;
  ThumbDIB:  TN_UDDIB;
...
end; // procedure TN_CMVideo4Form.CMVFGetThumbSize

// ************************************ TN_CMVideo4Form.CMVFSearchStatRecord ***
...
//
function TN_CMVideo4Form.CMVFSearchStatRecord
...
begin
  ...
  Result := Nil;
end; // function TN_CMVideo4Form.CMVFSearchStatRecord

// *************************************** TN_CMVideo4Form.CMVFAddStatRecord ***
...
//
// Search for Video Statistics Variant, defined TN_CMVideo4Form fields and
// if not found, add it to the end of table and return pointer to it. Otherwise
// (variant was found) return nil
//
// Video Setting Variants Table is stored in N_CM_VideoStat UDRarray
//
procedure TN_CMVideo4Form.CmBCameraButtonThresholdChange(Sender: TObject);
begin
  inherited;
  case CmBCameraButtonThreshold.ItemIndex of
  0:
    begin
      N_TempThreshold := 250;
    end;
  ...
  2:
    begin
      N_TempThreshold := 1000;
    end;
  3:
    begin
      N_TempThreshold := 1500;
    end;
  4:
    begin
      N_TempThreshold := 2000;
    end;
  5:
    begin
      N_TempThreshold := 3000;
    end;
  end;
end;

procedure TN_CMVideo4Form.CmBCaptureButtonChange(Sender: TObject);
begin
  inherited;
  if CmBCaptureButton.ItemIndex = 0 then
  begin
...
function TN_CMVideo4Form.CMVFAddStatRecord( AIsWorkingField: string )
...
end; // function TN_CMVideo4Form.CMVFAddStatRecord

// ************************************ TN_CMVideo4Form.CMVFSetIsWorkingField ***
// Set CMSRIsWorking of CMVFPCurStatRecord if it is not nil
//
//     Parameters:
// AIsWorkingField - content of CMSRIsWorking field to set
//
procedure TN_CMVideo4Form.CMVFSetIsWorkingField( AIsWorkingField: string );
begin
  if CMVFPCurStatRecord = Nil then
    Exit;

end; // end of TN_CMVideo4Form.CMVFSetIsWorkingField

// ********************************** TN_CMVideo4Form.CMVFThumbsDGridProcObj ***
...
//
procedure TN_CMVideo4Form.CMVFThumbsDGridProcObj( ADGObj: TN_DGridBase;
                                                                AInd: Integer );
var
...
end; // procedure procedure TN_CMVideo4Form.CMVFThumbsDGridProcObj

// **************************************** TN_CMVideo4Form.CMVFFreezeStream ***
...
//
procedure TN_CMVideo4Form.CMVFFreezeStream();
var
...
begin
  ...
  begin
    ...
    if VCOSampleGrabberFlag = True then
    begin
      ...
      GrabbedSample := VCOGrabSample2(); // Grab Image from Video Stream without using SampleGrabber
  end; // with CMVFVideoCapt4 do
  ...
  N_Dump1Str( 'Count = ' +IntToStr(CMVFTimerCount) );
  if ( GrabbedSample.DIBSize.X = 0 ) or ( GrabbedSample.DIBSize.Y = 0 ) or
  ( CMVFTimerCount < 1 ) then
  begin
    N_Dump1Str( 'Catched an errored sample slide' );
    CMVFRunStream();
    Exit;
  end;
  ...
  //CMVFFreezedSlide.P^.CMSDB.DCMModality := 'ES';
...
end; // procedure procedure TN_CMVideo4Form.CMVFFreezeStream

// ******************************************* TN_CMVideo4Form.CMVFRunStream ***
...
//
procedure TN_CMVideo4Form.CMVFRunStream();
var
  SError: string;
begin
  with CMVFVideoCapt4 do
...
end; // procedure procedure TN_CMVideo4Form.CMVFRunStream

// ***************************************** TN_CMVideo4Form.CMVFSaveAndCont ***
// Save Freezed Slide and continue showing live video
//
// Used in several controls handlers (Capture button, Space key, Pedal or Camera button)
//
procedure TN_CMVideo4Form.CMVFSaveAndCont();
begin
  if not CMVFIsFreezed then
    Exit;
  ...
  CMVFFreezedSlideInd   := -1;
end; // procedure procedure TN_CMVideo4Form.CMVFSaveAndCont

// ***************************************** TN_CMVideo4Form.CMVFSkipAndCont ***
// Skip (do not Save) Freezed Slide and continue showing live video
//
// Used in several controls handlers (Capture button, Space key, Pedal or Camera button)
//
procedure TN_CMVideo4Form.CMVFSkipAndCont();
var
  SError: string;
begin
  with CMVFVideoCapt4 do
  begin
    ...
    if VCOCheckRes( 190 ) then
    begin
      SError := IntToStr( VCOIError ) + ' - 4, Unexpected DirectShow Error5 - ' +
                                                                      VCOSError;
      N_Dump1Str( SError );
      K_CMShowMessageDlg( SError, mtWarning );
    end;
  ...
  //RecordTimer.Enabled := False;
  CMVFRunStream();
end; // procedure procedure TN_CMVideo4Form.CMVFSkipAndCont

// ******************************************** TN_CMVideo4Form.CMVFFPCBDown ***
...
//
procedure TN_CMVideo4Form.CMVFFPCBDown( AKey: Integer );
begin
...
end; // procedure TN_CMVideo4Form.CMVFFPCBDown

// ********************************************** TN_CMVideo4Form.CMVFFPCBUp ***
...
//
procedure TN_CMVideo4Form.CMVFFPCBUp( AKey: Integer );
begin
...
end; // procedure TN_CMVideo4Form.CMVFFPCBUp

// ******************************************** TN_CMVideo4Form.CMVFFindPWin ***
...
//
function TN_CMVideo4Form.CMVFFindPWin( ASlideInd: Integer ): Integer;
var
  i: Integer;
begin
  ...
  Result := -1; // not found
end; // end of function TN_CMVideo4Form.CMVFFindPWin

// ************************************** TN_CMVideo4Form.CMVFGetFreePWinInd ***
// Get first Free Preview Window Index (0-3)
//
function TN_CMVideo4Form.CMVFGetFreePWinInd(): Integer;
var
  i: Integer;
begin
...
end; // end of function TN_CMVideo4Form.CMVFGetFreePWinInd

// ******************************** TN_CMVideo4Form.CMVFLayoutPreviewWindows ***
...
//
procedure TN_CMVideo4Form.CMVFLayoutPreviewWindows( APreviewMode: Integer );
var
  WinSize: TPoint;
begin
...
end; // procedure TN_CMVideo4Form.CMVFLayoutPreviewWindows

// ********************************* TN_CMVideo4Form.CMVFDiscardFreezedSlide ***
// Clear all info about Freezed Slide and free it
//
// Used in CMVFSkipAndCont and in OnClose handler
//
procedure TN_CMVideo4Form.CMVFDiscardFreezedSlide();
var
  i: Integer;
...
begin
  ...
  CMVFFreezedSlideInd := -1;
end; // procedure TN_CMVideo4Form.CMVFDiscardFreezedSlide

// ********************************** TN_CMVideo4Form.CMVFUpdatePWinsContent ***
...
//
procedure TN_CMVideo4Form.CMVFUpdatePWinsContent();
...
end; // end of procedure TN_CMVideo4Form.CMVFUpdatePWinsContent

// ****************************************** TN_CMVideo4Form.CMVFClearPWins ***
// Clear CMVFPreviewSInds array (fill by -1 values)
//
procedure TN_CMVideo4Form.CMVFClearPWins();
var
  i: Integer;
begin
...
end; // end of procedure TN_CMVideo4Form.CMVFClearPWins

// ************************************* TN_CMVideo4Form.CMVFUnHighlightPWin ***
// UnHighlight given PWin if it is currently highlighted
//
//     Parameters:
// APWinIndex - Index of Preview Window to Highlight
//
procedure TN_CMVideo4Form.CMVFUnHighlightPWin( APWinIndex: Integer );
begin
...
end; // end of procedure TN_CMVideo4Form.CMVFUnHighlightPWin

// ************************************* TN_CMVideo4Form.CMVFCalcVideoAspect ***
...
//
procedure TN_CMVideo4Form.CMVFCalcVideoAspect( AStrSize: string );
var
  nx, ny: Integer;
  Str:    string;
begin
...
end; // end of procedure TN_CMVideo4Form.CMVFCalcVideoAspect

// **************************************** TN_CMVideo4Form.CurStateToMemIni ***
// Save Current Self State (ComboBox Items and others)
//
procedure TN_CMVideo4Form.CurStateToMemIni();
begin
  Inherited;
end; // end of procedure TN_CMVideo4Form.CurStateToMemIni

// **************************************** TN_CMVideo4Form.MemIniToCurState ***
// Load Current Self State (ComboBox Items and others)
//
procedure TN_CMVideo4Form.MemIniToCurState();
begin
  Inherited;
end; // end of procedure TN_CMVideo4Form.MemIniToCurState


//***************** Global Procedures  **************************

//**************************************************** N_CreateCMVideo4Form ***
// Create and return new instance of TN_CMVideo4Form
//
//     Parameters:
...
//
function N_CreateCMVideo4Form( AOwner: TN_BaseForm;
                               APProfile: TK_PCMVideoProfile ): TN_CMVideo4Form;
var
...
begin
  N_CMVideo4Form := TN_CMVideo4Form.Create( Application );
  Result := N_CMVideo4Form;

  with N_CMVideo4Form do
  begin
    ...
    begin
      ...
      cbCaptMode.ItemIndex     := Integer(CMVCMode);
    ...
    WrkVideoFName := K_ExpandFileName( '(#WrkFiles#)LastFilm.wmv' );
    ...
    CMVFVideoCapt4 := TN_VideoCaptObj4.Create3( VideoPanel.Handle,
                         Rect( 0, 0, 1, 1 ), WrkVideoFName, InitVideoFileSize );

    if ( CMVFPProfile.CMDPStrPar1 = '3' ) then // for Mode 3
    begin
      //N_DSEnumVideoSizes2( DevName, cbCaptSize.Items );
      CMVFVideoCapt4.VCOVCaptDevName := DevName;
      CMVFVideoCapt4.VCOPrepInterfaces2();

      CMVFVideoCapt4.VCOEnumVideoSizes( cbCaptSize.Items, cbFormat.Items );
    end;

    with CMVFVideoCapt4 do // set VCONeededVideoCMST
    begin
    ...
    end; // with CMVFVideoCapt4 do // set VCONeededVideoCMST
  ...
  N_Dump1Str( 'End of N_CreateCMVideo4Form' );
end; // function N_CreateCMVideo4Form
