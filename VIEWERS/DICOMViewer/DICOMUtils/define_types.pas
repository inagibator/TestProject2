...
{$ENDIF}
  ...
  public
    ...
    SiemensInterleaved { 0=no,1=yes,2=not defined } , SiemensSlices,
      SiemensMosaicX, SiemensMosaicY, CompressSz, CompressOffset, SeriesNum,
      AcquNum, ImageNum, Monochrome, SamplesPerPixel, PlanarConfig, ImageStart,
      little_endian, Allocbits_per_pixel, Storedbits_per_pixel, ImageSz,
      accession, PatientIDint, VolumeNumber, WindowWidth, WindowCenter,
      GenesisPackHdr, NamePos, StudyDatePos, MinIntensity, MaxIntensity,
      RLEredOffset, RLEgreenOffset, RLEblueOffset, RLEredSz, RLEgreenSz,
      RLEblueSz: integer; { must be 32-bit integer aka longint }
    AcqTime, ImgTime, PatientName, PatientID, StudyID, StudyDate, StudyTime, modality,
    serietag: string;
    StudyDescription, SeriesDescription: string;
    StudyUID, SerieUID: string;
    InstitutionName, PhysicianName: string;
    PatientBirthDate, PatientAge, PatientSex: string;
  end;
...
// swap 32-bit integer between long/little endian
procedure ClearHdr(var lHdr: AHdr);
procedure DICOM2AnzHdr(var lBHdr: AHdr; lAnonymize: boolean;
  var lFilename: string; var lDICOMdata: TDICOMdata);
FUNCTION specialsingle(var s: single): boolean;
// check if 32-bit float is Not-A-Number, infinity, etc
...
begin
  ...
  if length(lFilePath) < 1 then
    exit;
  if lFilePath[length(lFilePath)] <> PathDelim then
    lFilePath := lFilePath + PathDelim;
  // Delphi3 bug: sometimes forgets pathdelim
  result := lFilePath;
end;
...
begin
  ...
  if lInName[lInc] = PathDelim then
    inc(lInc); // if '\folder' then return 'folder'
...
begin
  FSize := 0;
  if not FileExistsEX(lFName) then
    exit;
...
begin
  ...
  begin
    ...
    if lLen > 10 then
      lLen := 10; // 10=size of patient_ID array
    if lLen > 0 then
    ...
    if lLen > 80 then
      lLen := 80; // 80=size of descrip array
    if lLen > 0 then
      for lInc := 1 to lLen do
    ...
    if lLen > 10 then
      lLen := 10; // 10=size of exp_date array
    if lLen > 0 then
      for lInc := 1 to lLen do
    ...
    if lLen > 10 then
      lLen := 10; // 10=size of exp_time array
    if lLen > 0 then
      for lInc := 1 to lLen do
    ...
    if lLen > 10 then
      lLen := 10; // 10=size of generated array
    if lLen > 0 then
      for lInc := 1 to lLen do
  ...
  if lDICOMdata.Allocbits_per_pixel <> 8 then
  begin
    if lDICOMdata.Allocbits_per_pixel = 32 then
    begin
      lBHdr.bitpix := 32;
    ...
    else if lDICOMdata.Allocbits_per_pixel = 64 then
    begin
      lBHdr.bitpix := 64;
      lBHdr.datatype := 64;;
    end
    else
    begin // 16bits per pixel
      lBHdr.bitpix := 16;
      lBHdr.datatype := 4;
    end;
  end;
