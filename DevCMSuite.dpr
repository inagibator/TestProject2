...
uses
  ...
  K_CMSCom in 'K_CMSCom.pas' {D4WCMServer: CoClass},
  ...
  try
    ...
    begin
      ...
      begin
        ...
        K_CMShowMessageDlg( //sysout
          'Application terminated by exception:'+#13#10 + E.Message, mtError );
...
//
  ...
  Application.CreateForm(TN_PMTMain5Form, N_PMTMain5Form);
  Application.CreateForm(TN_PMTVizForm, N_PMTVizForm);
  Application.CreateForm(TN_PMTHelpForm, N_PMTHelpForm);
  Application.Run;
    Exit;
...
//        Application.Icon.LoadFromFile( FName );
  ...
  begin
      ////////////////////////////////////////
      // Check DSN in command line
      //
        ParInd := K_CMDParams.IndexOfName('DSN');
        if ParInd >= 0 then
        begin
          DSN := K_CMDParams.ValueFromIndex[ParInd];
          N_Dump1Str('Set DSN=' + DSN);
          N_StringToMemIni( 'CMSDB', 'DSN', DSN );
        end; // if ParInd >= 0 then
      //
      ////////////////////////////////////////

      ////////////////////////////////////////
      // Check Install Set Info
      //
        ParInd := K_CMDParams.IndexOfName( 'IniSetInfo' );
        if ParInd >= 0 then
        begin
          K_CMInstallIniSetInfo( K_CMDParams.ValueFromIndex[ParInd] );
          goto LExit;
        end; // if ParInd >= 0 then
      //
      ////////////////////////////////////////
      ...
      ////////////////////////////////////////
      // Check Install Get Config Info
      //
        ParInd := K_CMDParams.IndexOfName( 'MFConfigGetInfo' );
        if ParInd >= 0 then
        begin
          K_CMEDAccess := TK_CMEDDBAccess.Create;
          with TK_CMEDDBAccess(K_CMEDAccess) do
          begin
            EDAConfigGetInfo( K_CMDParams.ValueFromIndex[ParInd] );
...
//!!! K_CMRemoveOldDump2HaltFiles() in K_CMEDAccess destructor
      ...
      ////////////////////////////////////////
      // Check Install Set Config Info
      //
        ParInd := K_CMDParams.IndexOfName( 'MFConfigSetInfo' );
        if ParInd >= 0 then
        begin
          K_CMEDAccess := TK_CMEDDBAccess.Create;
          with TK_CMEDDBAccess(K_CMEDAccess) do
          begin
            EDAConfigSetInfo( K_CMDParams.ValueFromIndex[ParInd] );
//!!! Need to separate Free and Nil because of
//!!! K_CMRemoveOldDump2HaltFiles() in K_CMEDAccess destructor
            Free;
            K_CMEDAccess := nil;
...
//      N_MainModalForm := N_CM_MainForm;
      ...
      Application.CreateForm(TN_CML1Form, N_CML1Form);
      K_SetFFCompCurLangTexts( N_CML1Form );
      K_PrepLLLCompTexts( N_CML1Form );
...
//  Remove Old Dump Data for All CMS applications (CMSite, CMScan, CMSupport)
  ...
  N_AddStrToFile( '***** Run process: Application Finish '#13#10 );
