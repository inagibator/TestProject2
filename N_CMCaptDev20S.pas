unit N_CMCaptDev20S;
// EzSensor(MediaRay+) 2 device

// 18.05.16 - changing the device 12 files for new libraries
// 22.08.16 - implemented a native log
// 31.05.17 - new filters
// 15.07.17 - remembering new sensors' state
// 04.08.17 - fixed a crashing while updating the device to 3.114
// 13.08.17 - fixed a crashing after an update, was not fixed last time
// 22.08.17 - new filters component was no anchored properly
// 08.11.17 - changed interface (narrow), added N_ShowForm constant to open the form if = true
// 30.11.17 - new pixel size algorythm
// 12.11.18 - new letters for new sensors serials
...
const
 // acquisition flags tVDACQ_CallBackRec.rFlags
  ...
  N_ShowForm = FALSE; // showing a form without a device
...
type TN_VDEnu_IOra = record
  ...
  rModelTag: integer;         // <0:FW_err 0:N/A 1:"1.0" 2:"1.5" 3:"2.0"
  rReservInt: array [0..3] of integer;  // reserved
  rSerialId: array [0..31] of AnsiChar; // proprietary serial ID from detector's memory
  rModelTit: array [0..31] of AnsiChar; // model's title
  rIniName:  array [0..191] of AnsiChar; // suggested path of configuration file
  ...
  rReservStr: array [0..63] of AnsiChar; // reserved
...
type TN_CMCDServObj20 = class(TK_CMCDServObj)
...
end; // end of type TN_CMCDServObj20 = class( TK_CMCDServObj )
...
var
  ...
  N_CMEDVD_IntParam:                  TN_stdcallIntFuncPtrInt;

  N_CMCDServObj20: TN_CMCDServObj20;
...
uses
  SysUtils, Dialogs,
  K_CLib0, N_Gra2,
  N_Lib1, N_CM1, N_Lib0, N_CMCaptDev20SF;

var
  CaptForm: TN_CMCaptDev20Form;
...
begin
  ...
  begin
    N_Dump1Str('Error with acquisition by MediaRay+ 2');
...
//
function TN_CMCDServObj20.CDSInitAll() : Integer;
var
...
begin
  ...
  begin
    ...
    DllFName := 'C:\EzSensor\EzSensorEnuM.dll';
  ...
  begin
    ...
    FuncAnsiName := 'VDEIOrM_EnumConnected';
    ...
    FuncAnsiName := 'VD_IntParam';
    N_Dump1Str( 'GetProcAddress ' + String(FuncAnsiName) );
    N_CMEDVD_IntParam := GetProcAddress( CDSDllHandle, @FuncAnsiName[1] );
    if not Assigned(N_CMEDVD_IntParam) then ReportError();
  ...
  N_Dump1Str( 'MediaRay+ 2 >> CDSInitAll end' );
end; // procedure N_CMCDServObj20.CDSInitAll

//********************************************* TN_CMCDServObj20.CDSFreeAll ***
// Close Device and Free all resources
//
//     Parameters
// Result - return 0 if OK
//
function TN_CMCDServObj20.CDSFreeAll(): Integer;
begin
  ...
  Result := 0; // freed OK
end; // procedure TN_CMCDServObj20.CDSFreeAll

//**************************************** N_CMCDServObj20.CDSCaptureImages ***
// Capture images
//
//    Parameters
...
//
procedure TN_CMCDServObj20.CDSCaptureImages( APDevProfile: TK_PCMDeviceProfile;
                                             var ASlidesArray: TN_UDCMSArray );
begin
{
  PProfile := APDevProfile;
  N_Dump1Str( 'MediaRay+ 2 >> CDSCaptureImages begin' );

  if CDSInitAll() > 0 then // no device driver installed
    Exit;

  SetLength(ASlidesArray, 0);
  CaptForm          := TN_CMCaptDev20Form.Create(application);
  CaptForm.ThisForm := CMCaptDev20Form;
  ...
  begin
    ...
    N_Dump1Str( 'MediaRay+ 2 >> CDSCaptureImages before ShowModal' );

    ShowModal();

    CDSFreeAll();
    N_Dump1Str( 'MediaRay+ 2 >> CDSCaptureImages after ShowModal' );
  ...
  N_Dump1Str( 'MediaRay+ 2 >> CDSCaptureImages end' );
end; // procedure N_CMCDServObj20.CDSCaptureImages

//********************************* TN_CMCDServObj20.CDSStartCaptureToStudy ***
...
//
function TN_CMCDServObj20.CDSStartCaptureToStudy(  APDevProfile: TK_PCMDeviceProfile; var AStudyDevCaptAttrs : TK_CMSDCDAttrs ): TK_CMStudyCaptState;
begin
  ...
  N_Dump1Str( 'MediaRay+ 2 >> CDSStartCaptureToStudy start Res=' + IntToStr(Ord(Result)) );

end; // function TN_CMCDServObj20.CDSStartCaptureToStudy

//******************************** TN_CMCDServObj20.CDSGetDefaultDevIconInd ***
...
//
function TN_CMCDServObj20.CDSGetDefaultDevIconInd( const AProductName : string): Integer;
begin
  Result := 37;
end; // function TN_CMCDServObj20.CDSGetDefaultDevIconInd

//********************************* TN_CMCDServObj20.CDSGetDefaultDevDCMMod ***
...
// Result - Returns device Icon default Index
//
function TN_CMCDServObj20.CDSGetDefaultDevDCMMod( const AProductName : string ): string;
begin

    Result := 'IO';

end; // function TN_CMCDServObj20.CDSGetDefaultDevDCMMod

// destructor N_CMCDServObj20.Destroy
//
destructor TN_CMCDServObj20.Destroy();
begin
  //CDSFreeAll();
end; // destructor N_CMCDServObj20.Destroy

//**************************************** TN_CMCDServObj20.FCapturePrepare ***
// Capture Prepare
//
//     Parameters
// APDevProfile - pointer to profile
...
//
procedure TN_CMCDServObj20.FCapturePrepare( APDevProfile: TK_PCMDeviceProfile; var ASlidesArray: TN_UDCMSArray );
begin
  PProfile := APDevProfile;
  if CDSInitAll() > 0 then // no device driver installed
    Exit;
...
end; // procedure TN_CMCDServObj20.FCapturePrepare

Initialization

// Create and Register MediaRay+ 2 Service Object
N_CMCDServObj20 := TN_CMCDServObj20.Create( N_CMECD_MediaRayPlus2_Name );

with K_CMCDRegisterDeviceObj( N_CMCDServObj20 ) do
begin
  CDSCaption := 'MediaRay+ 2';
  IsGroup := False;
  ShowSettingsDlg := True;
end; // with K_CMCDRegisterDeviceObj( N_CMCDServObj20 ) do
