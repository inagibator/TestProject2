...
type
  TK_FormCMUTLoadDBData1 = class(TN_BaseForm)
    GBAll: TGroupBox;
...
var
  K_FormCMUTLoadDBData1: TK_FormCMUTLoadDBData1;

procedure K_CMUTLoadDBData1Dlg();

implementation

uses K_CLib0, K_CM0, K_CM1, K_FCMSupport,
...
begin
  ...
  K_FormCMUTLoadDBData1 := TK_FormCMUTLoadDBData1.Create(Application);
  with K_FormCMUTLoadDBData1 do
  begin
    ...
    N_Dump1Str( 'DAD>> Before K_FormCMUTLoadDBData1 show' );
    ShowModal();
    N_Dump1Str( 'DAD>> After K_FormCMUTLoadDBData1 show' );
    ...
    K_FormCMUTLoadDBData1 := nil;
  end;

end; // K_CMUTLoadDBData1Dlg

procedure TK_FormCMUTLoadDBData1.PathChange;
begin
...
procedure TK_FormCMUTLoadDBData1.RebuildPathSettings;
begin
...
procedure TK_FormCMUTLoadDBData1.FormShow(Sender: TObject);
begin
  PathChange();
...
procedure TK_FormCMUTLoadDBData1.BtOpenClick(Sender: TObject);
var
 i, N : Integer;
begin
  // Parse Report File
  ...
  begin
    ...
    N := AllExistedFList.IndexOf('');
    if N > 0 then
    for i := AllExistedFList.Count - 1 downto N do
      AllExistedFList.Delete( i );

    if Pos( 'All is OK!!!', AllExistedFList[AllExistedFList.Count - 1] ) = 0 then
    begin
...
procedure TK_FormCMUTLoadDBData1.ChBLoadMediaClick(Sender: TObject);
begin
...
procedure TK_FormCMUTLoadDBData1.BtLoadClick(Sender: TObject);
var
  ExtFilePat : string;
  ErrRootPath : string;
  ImgRootPath : string;
  ...
  SQLStr : string;
  FilesPattern : string;
  N, i, j, OldPatID, NewPatID, ICount, VCount, I3Count, SCount : Integer;
  ...
  begin
    ...
    begin
      ...
      Open();
      N_Dump1Str( format( 'DAD>> %d records from %s are added to %s (%d)', [N1,ATableName,ATableName1,Fields[0].AsInteger] ) );
      Close();
    end;
  end;
  ...
  begin
    ...
    RLogFName := MergeInfoPath + 'T2\Logs\' + ExtractFileName(LogFName);
    ...
    begin
      ...
      begin
        ...
        if K_CMEDDBVersion >= 40 then // Set ID to NextSlideID sequence
          SQLStr := SQLStr +
                       'EXECUTE IMMEDIATE string( ''ALTER SEQUENCE dba.NextSlideID RESTART WITH '', @MinID );' + #10;
...
// 2020-07-28 add Capt3DDevObjName <> ''
  ...
  //              K_CopyFile( TmpStrings[j], DstBasePath + NewFName );
      ...
      K_FormCMSupport.ShowInfo( format( '%d Image, %d Study, %d Video files and %d 3D folders for %d Media Objects are added',
                                        [ICount, SCount, VCount, I3Count, N] ), 'TK_FormCMUTLoadDBData.BtLoadClick >>' );
      K_AddStringToTextFile( MergeReportFName,
         format( 'Total to Target:'#13#10 +
                 '%6d DB media objects,'#13#10 +
                 '%6d corresponding Files and 3D folders.',
                   [N, ICount + SCount + VCount + I3Count] ) );
      ...
      /////////////////////////////////////////////
       ...
       if (FSAfterExtra = FSBeforeExtra) and
          ...
          (FSAfterExisted - FSBeforeExisted = ICount + SCount + VCount + I3Count) then
        ...
        begin
          ...
          if FSAfterExisted - FSBeforeExisted = ICount + SCount + VCount + I3Count then
...
procedure TK_FormCMUTLoadDBData1.CurStateToMemIni;
begin
  inherited;
...
procedure TK_FormCMUTLoadDBData1.MemIniToCurState;
begin
  inherited;
...
procedure TK_FormCMUTLoadDBData1.ShowProgress( AllCount, CurCount: Integer);
begin
