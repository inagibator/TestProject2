...
// 16.08.18 - wrk to tmp, new statuses
// 05.09.18 - 3d access test added
// 05.11.18 - settings interface changed
// 2018.11.07 CDSGetDefaultDevIconInd function added
// 30.11.18 - fixed a minor code commentary misspell
...
uses
  ...
  K_CM0, K_CMCaptDevReg, N_Types, N_CMCaptDev0, N_CMCaptDev28F,
  N_CML2F, N_CMCaptDev28aF;

type TN_CMCDServObj28 = class(TK_CMCDServObj)
...
end; // end of type TN_CMCD ServObj28 = class( TK_CMCDServObj )

var
  N_CMCDServObj28: TN_CMCDServObj28;
  CMCaptDev28Form: TN_CMCaptDev28Form;
...
//
function TN_CMCDServObj28.CDSInitAll(AGroupName, AProductName: string )
                                                                      : Integer;
begin
...
//********************************************** TN_CMCDServObj28.CDSFreeAll ***
// Close Device and Free all resources
//
//     Parameters
// Result - return 0 if OK
//
function TN_CMCDServObj28.CDSFreeAll(): Integer;
begin
  ...
  Result := 0; // freed OK
end; // procedure TN_CMCDServObj28.CDSFreeAll

//**************************************** N_CMCDServObj28.CDSCaptureImages ***
// Capture images
//
//    Parameters
...
//
procedure TN_CMCDServObj28.CDSCaptureImages( APDevProfile: TK_PCMDeviceProfile;
...
begin
  ...
  CMCaptDev28Form          := TN_CMCaptDev28Form.Create(Application);
  CMCaptDev28Form.CMOFThisForm := CMCaptDev28Form;

  with CMCaptDev28Form, APDevProfile^ do
  begin
...
end; // procedure N_CMCDServObj28.CDSCaptureImages

//******************************************** TN_CMCDServObj28.StartDriver ***
// Start C++ driver
//
//    Parameters
...
//
function TN_CMCDServObj28.StartDriver( Path: string; Bit: Boolean ): Boolean;
var
...
begin
 ...
 // end
  ...
  WriteLn (f, '<Name value="' + K_CMGetPatientDetails( K_CMEDAccess.CurPatID, '(#PatientFirstName#) (#PatientSurname#)' ) + '"/>' );
  ...
  if K_CMGetPatientDetails( K_CMEDAccess.CurPatID, '(#PatientDOB#)' ) <> '' then // if no error in a date
  begin
    DOB := StrToDateTime(K_CMGetPatientDetails( K_CMEDAccess.CurPatID, '(#PatientDOB#)' ) +
                                                                ' 00:00', Fmt );
  end
  else
  begin
  ...
  if K_CMGetPatientDetails( K_CMEDAccess.CurPatID, '(#PatientGender#)' ) <> '' then
    WriteLn (f, '<Sex value="' + K_CMGetPatientDetails( K_CMEDAccess.CurPatID, '(#PatientGender#)' ) + '"/>')
  else
    WriteLn (f, '<Sex value="O"/>'); // default

  WriteLn (f, '</PatientInfo>');
  WriteLn (f, '<AcquisitionInfo>');
  WriteLn (f, '<Modality value="CT"/>');
  WriteLn (f, '<OutPath value="C:\Ray\ImageOutput.xml"/>');
  WriteLn (f, '<ImageFormat value="DCM"/>');
...
begin
  ...
  Result := True;
end; // function TN_CMCDServObj28.StartDriver

//********************************************* TN_CMCDServObj28.SendExtMsg ***
...
//
procedure TN_CMCDServObj28.SendExtMsg( Cmd, Param1, Param2, Param3: Integer );
begin
end; // procedure SendExtMsg

//***************************************** TN_CMCDServObj28.CDSSettingsDlg ***
// Show Special Seting Dialog
//
//     Parameters
// APDevProfile - Pointer to Device Profile record
//
procedure TN_CMCDServObj28.CDSSettingsDlg( APDevProfile: TK_PCMDeviceProfile );
var
  Form: TN_CMCaptDev28aForm;
begin
  PProfile := APDevProfile;
...
end; // procedure TN_CMCDServObj28.CDSSettingsDlg

//************************************* TN_CMCDServObj28.CDSGetGroupDevNames ***
// Get RayScan Devices Names
//
//     Parameters
// AGroupDevNames - given Strings object to fill
// Result         - number of all Devices Names
//
function TN_CMCDServObj28.CDSGetGroupDevNames( AGroupDevNames: TStrings ):
                                                                        Integer;
begin
  //AGroupDevNames.Add( N_CMECD_ClearVision );

  Result := AGroupDevNames.Count;
end; // function TN_CMCDServObj28.CDSGetGroupDevNames

// destructor N_CMCDServObj28.Destroy
//
destructor TN_CMCDServObj28.Destroy();
begin
  //CDSFreeAll();
end; // destructor N_CMCDServObj28.Destroy

//******************************** TN_CMCDServObj28.CDSGetDefaultDevIconInd ***
...
//
function TN_CMCDServObj28.CDSGetDefaultDevIconInd( const AProductName : string): Integer;
begin
  Result := 42;
end; // function TN_CMCDServObj28.CDSGetDefaultDevIconInd

//********************************* TN_CMCDServObj28.CDSGetDefaultDevDCMMod ***
...
// Result - Returns device Icon default Index
//
function TN_CMCDServObj28.CDSGetDefaultDevDCMMod( const AProductName : string ): string;
begin
  Result := 'CT';
end; // function TN_CMCDServObj28.CDSGetDefaultDevDCMMod


Initialization

// Create and Register RayScan Service Object
N_CMCDServObj28 := TN_CMCDServObj28.Create( N_CMECD_RayScanCBCT_Name );

with K_CMCDRegisterDeviceObj( N_CMCDServObj28 ) do
begin
  CDSCaption := 'RayScan CBCT';
  IsGroup := False;
  ShowSettingsDlg := True;
end; // with K_CMCDRegisterDeviceObj( N_CMCDServObj28 ) do
