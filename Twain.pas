...
const
  TWON_PROTOCOLMINOR = 1;    { Changed for Version 2.1 }
...
{ TWON_ARRAY. Container for array of values (a simplified TW_ENUMERATION) }
  TW_ARRAY = record
    ...
    // LM: $ALIGN 2 specified. ..0 restored
  end;
...
{ DAT_EVENT. For passing events down from the application to the DS. }
  ...
  { LM: Item is declared as member of the variant part of the structure
    because pascal wants the variant part be the last.
    See http://rvelthuis.de/articles/articles-convert.html for an
    explanation of this trick }
  TW_INFO = record
    InfoID    : TW_UINT16;
    ItemType  : TW_UINT16;
    case NumItems  : TW_UINT16 of
      0: (CondCode  : TW_UINT16); { Depreciated, use ReturnCode. TWAIN 2.0 and older. }
      1: (ReturnCode: TW_UINT16;  { TWAIN 2.1 and newer }
          Item      : TW_UINTPTR);
  end;
  {$EXTERNALSYM TW_INFO}
  pTW_INFO = ^TW_INFO;
...
{ DAT_STATUS. Application gets detailed status info from a data source with this. }
  TW_STATUS = record
    case ConditionCode : TW_UINT16 of { Any TWCC_ constant }
...
{ ICAP_SUPPORTEDBARCODETYPES and TWEI_BARCODETYPE values Added 1.7 }
  ...
  TWBT_2OF5INDUSTRIAL       = 13;     { Added 1.8 }
  {$EXTERNALSYM TWBT_2OF5INDUSTRIAL}
  TWBT_2OF5MATRIX           = 14;     { Added 1.8 }
  {$EXTERNALSYM TWBT_2OF5MATRIX}
  TWBT_2OF5DATALOGIC        = 15;     { Added 1.8 }
  {$EXTERNALSYM TWBT_2OF5DATALOGIC}
  TWBT_2OF5IATA             = 16;     { Added 1.8 }
  {$EXTERNALSYM TWBT_2OF5IATA}
  TWBT_3OF9FULLASCII        = 17;     { Added 1.8 }
  {$EXTERNALSYM TWBT_3OF9FULLASCII}
  TWBT_CODABARWITHSTARTSTOP = 18;     { Added 1.8 }
  {$EXTERNALSYM TWBT_CODABARWITHSTARTSTOP}
  TWBT_MAXICODE             = 19;     { Added 1.8 }
...
{ CAP_LANGUAGE Language Constants }
  TWLG_DAN    = TWLG_DANISH;           { use TWLG_DANISH instead }
  {$EXTERNALSYM TWLG_DAN}
  TWLG_DUT    = TWLG_DUTCH;            { use TWLG_DUTCH instead }
  {$EXTERNALSYM TWLG_DUT}
  TWLG_ENG    = TWLG_ENGLISH;          { use TWLG_ENGLISH instead }
  {$EXTERNALSYM TWLG_ENG}
  TWLG_USA    = TWLG_ENGLISH_USA;      { use TWLG_ENGLISH_USA instead }
  {$EXTERNALSYM TWLG_USA}
  TWLG_FIN    = TWLG_FINNISH;          { use TWLG_FINNISH instead }
  {$EXTERNALSYM TWLG_FIN}
  TWLG_FRN    = TWLG_FRENCH;           { use TWLG_FRENCH instead }
  {$EXTERNALSYM TWLG_FRN}
  TWLG_FCF    = TWLG_FRENCH_CANADIAN;  { use TWLG_FRENCH_CANADIAN instead }
  {$EXTERNALSYM TWLG_FCF}
  TWLG_GER    = TWLG_GERMAN;           { use TWLG_GERMAN instead }
  {$EXTERNALSYM TWLG_GER}
  TWLG_ICE    = TWLG_ICELANDIC;        { use TWLG_ICELANDIC instead }
  {$EXTERNALSYM TWLG_ICE}
  TWLG_ITN    = TWLG_ITALIAN;          { use TWLG_ITALIAN instead }
  {$EXTERNALSYM TWLG_ITN}
  TWLG_NOR    = TWLG_NORWEGIAN;        { use TWLG_NORWEGIAN instead }
  {$EXTERNALSYM TWLG_NOR}
  TWLG_POR    = TWLG_PORTUGUESE;       { use TWLG_PORTUGUESE instead }
  {$EXTERNALSYM TWLG_POR}
  TWLG_SPA    = TWLG_SPANISH;          { use TWLG_SPANISH instead }
  {$EXTERNALSYM TWLG_SPA}
  TWLG_SWE    = TWLG_SWEDISH;          { use TWLG_SWEDISH instead }
