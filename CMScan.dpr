...
//        Application.Icon.LoadFromFile( FName );
  ...
  begin
      if ParamStr(1) = '/ScanSelfUpdate' then
      begin
        K_CMScanSelfUpdate();
        goto LExit;
      end;
      N_Dump1Str( '***** Application mode ClientScan' );
      Application.CreateForm( TK_FormCMScan, K_FormCMScan );
      Application.ShowMainForm := FALSE;
      Application.OnException := K_FormCMScan.OnUHException;
      OnExcept := K_FormCMScan.OnUHException;

      // Create CMS UI Actions container form
      Application.CreateForm(TN_CMResForm, N_CMResForm);

      K_CMScanInit();
      if K_FormCMScan = nil then goto LExit;
